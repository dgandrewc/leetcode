...
const solve = input => {
    ...
    let m=input[0][1]*1
    let map=[]
    let qr=new queue, qb=new queue, qc=new queue, ql=new queue, qs=new queue
    ...
    while(qr.size!=0)
    {
        let r=qr.pop(), b=qb.pop(), c=qc.pop(), l=ql.pop(), s=qs.pop()
        ...
        for(let i=0; i<4; i++)
        {
            ...
            if(dx[i]==0 && (l+2)%4!=i) //y간 이동의 경우 && 이전 이동경로 방지
            {
                ...
                {
                    if(map[rx][j]=='O' && flag3!=1) {console.log(c);console.log(s+dir[i]); return} //빨간공이 빠지면 끝
                ...
                if(flag3==0)
                {
                    qr.push([rx, j]); qb.push([bx, k]); qc.push(c+1); ql.push(i), qs.push(s+dir[i])
                }
            }
            else if(dy[i]==0 && (l+2)%4!=i) //x간 이동의 경우 && 이전 이동경로 방지
            {
                ...
                {
                    if(map[j][ry]=='O' && flag3!=1) {console.log(c);console.log(s+dir[i]); return} //빨간공이 빠지면 끝
                ...
                if(flag3==0)
                {
                    qr.push([j, ry]); qb.push([k, by]); qc.push(c+1); ql.push(i); qs.push(s+dir[i])
                }
            }
        }
    }
